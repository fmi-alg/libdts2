...
constexpr uint64_t minNeededBits(uint64_t v) {
	if (v == 0 || v == 1) {
		return 1;
	}
	else {
		return 1+minNeededBits(v/2);
	}
}

constexpr std::pair<const uint64_t, const bool> autoint_add_upper_bound(const uint64_t up1, const bool upl1, const uint64_t up2, const bool upl2) {
	using return_type = std::pair<const uint64_t, const bool>;
	if (upl1 && upl2) {
		...
		if (int64_t(up1)+int64_t(up2) <= std::numeric_limits<int32_t>::max()) { //fits int
			...
			return return_type(minNeededBits(up1+up2), true);
		}
	}
	else if (upl1 && !upl2) {
		if (up1 < 63) {
			const uint64_t up1_no_log = (static_cast<uint64_t>(1) << up1)-1;
			const uint64_t result_up_no_log = up1_no_log + up2;
			if (result_up_no_log < up1_no_log) { //overflow should not happen
				...
				if (result_up_no_log <= std::numeric_limits<int32_t>::max()) {
					...
					return return_type(minNeededBits(result_up_no_log), true);
				}
			}
		}
		else {
...
constexpr std::pair<const uint64_t, const bool> autoint_mult_upper_bound(const uint64_t up1, const bool upl1, const uint64_t up2, const bool upl2) {
	using return_type = std::pair<const uint64_t, const bool>;
	if (upl1 && upl2) {
		...
		if (up1 < 63) { //up2 is at most 2^31-1 thus we can detect an overflow
			...
			if (result_up_no_log < up1_no_log) { //overflow
				return return_type(up1+minNeededBits(up2), true);
			}
			else {
				if (result_up_no_log <= std::numeric_limits<int32_t>::max()) {
					return return_type(result_up_no_log, false);
